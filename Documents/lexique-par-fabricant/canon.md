...
| Ce qu'on dit en formation | Sur votre Canon |
|---|---|
| **`AF-S`** — sujet immobile | **`One Shot`** ou `One-Shot AF` |
| **`AF-C`** — sujet qui bouge | **`AI Servo`** *(reflex)* ou `Servo AF` *(série R)* |
| L'appareil choisit lequel des deux | `AI Focus` |
| Mise au point manuelle | `MF` |
| **Un seul collimateur, que je déplace** | `1 collimateur` · `AF Spot` · `Zone AF flexible` |
| Une petite zone | `Zone AF` |
...
| Limite haute de l'ISO auto | `Plage ISO auto` |
| Vitesse la plus lente acceptée | `Vit. obtu. min.` |
...
**Votre menu rapide, c'est le bouton `Q`.** Apprenez-le avant tout le reste : il affiche la plupart de ces réglages sur un seul écran, sans entrer dans les menus.

| Réglage | Chemin |
|---|---|
| **Qualité d'image / RAW** | Onglet rouge `Prise de vue 1` → `Qualité d'image` → **`RAW+JPEG`** |
| **Mode de mise au point** | Onglet violet `AF` → `Fonctionnement AF` — ou le bouton `AF` |
| **Zone AF** | Onglet `AF` → `Zone AF` |
| **Détection visage / œil** | Onglet `AF` → `Détection œil` ou `Sujet à détecter` |
| **Limite de l'ISO auto** | Onglet rouge → `Sensibilité ISO` → `Plage ISO auto` |

*Canon range ses menus par onglets de couleur : **rouge** = prise de vue, **violet** = autofocus, **jaune** = configuration.*

**Vos fichiers RAW portent l'extension `.CR3`** — ou `.CR2` sur les boîtiers plus anciens.
