...
- [x] 1.1 In `src/src/content/lecons/1.yaml`, remove the `reflex` field from the
      `designer-le-point` écran (bloc `mise-au-point`).
- [x] 1.2 Re-read the `autofocus` écran's closing bullet (slide 10) and confirm it still reads
