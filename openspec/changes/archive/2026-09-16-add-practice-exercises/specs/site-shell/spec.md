...
The site SHALL provide a single home page from which every lesson, the lexique, every printable
reference sheet, and every session's practice-exercise page is reachable in one click, plus the
course description aimed at prospective club members.
...
- **THEN** the three lessons, the lexique, the reference sheet index, and the three sessions'
  practice-exercise pages are each reachable by one link
- **AND** the three lessons are presented as peers, none marked current, dated, or otherwise
  privileged
...
- **AND** the lexique, reference, and exercise sections are reachable without entering a lesson
...
#### Scenario: A session's exercise page is offered on the hub

- **WHEN** a session's practice-exercise page is listed on the hub
- **THEN** it is presented with the same card treatment as that session's lesson, carrying its
  séance ordinal, a title, and one line stating what the page is for
- **AND** a participant can tell the destination is a set of practice prompts, not the lesson
...
- **THEN** they occupy the same height, whether or not their titles wrap to a different number of
  lines
