...
- [ ] 2.7 Replace the branchy single-lens/two-lens `exposition` text with one generated
      ...
      generated string.
- [ ] 2.8 Verify manually (dev server, phone-width and full-width) that both panels' controls
...
- [ ] 3.1 Update `le-f-est-une-division` in `src/src/content/lecons/1.yaml`: new `claim` (naming
      ...
      claim.
- [ ] 3.2 Run `node src/scripts/verifier-contenu-final.mjs` (or the project's equivalent content
      check) and verify it passes against the updated screen.
- [ ] 3.3 Run the Astro build (`npm run build` from `src/`) and verify it succeeds with no
      errors referencing `le-f-est-une-division` or `OuvertureDivision`.

## 4. Spec sync

- [ ] 4.1 Run `openspec validate simplify-aperture-division-simulator --strict` and verify it
      passes.
